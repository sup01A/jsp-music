				...
				<section class="table-responsive">
					<table class="table table-striped">
						...
						<c:forEach items="${list }" var="Album" varStatus="status">
							<tr>
								<td>${status.count }</td>
								<td><a href="<%=contextPath %>/AlbumServlet?info=play&playId=${Album.albumId}" target="_blank"><c:out value="${Album.albumTitle }"></c:out></a></td>
								<td><a href="<%=contextPath %>/SingerServlet?info=play&playId=${Album.singerId}" target="_blank"><c:out value="${Album.singer.singerName }"></c:out></a></td>
								<td><a href="<%=contextPath %>/AlbumServlet?info=removeFollow&albumId=${Album.albumId}">删除</a></td>
							</tr>
