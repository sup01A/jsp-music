			...
			<div class="form">
				...
				<section class="table-responsive">
					<table class="table table-striped">
						...
						<c:forEach items="${list }" var="Singer" varStatus="status">
							<tr>
								<td>${status.count }</td>
								<td><a href="<%=contextPath %>/SingerServlet?info=play&playId=${Singer.singerId}" target="_blank"><c:out value="${Singer.singerName }"></c:out> </a></td>
								<td>
									<c:if test="${Singer.singerSex==0 }">
										保密
									</c:if>
								...
								<td><a href="<%=contextPath %>/SingerServlet?info=removeFollow&singerId=${Singer.singerId}">删除</a></td>
							</tr>
						</c:forEach>
					</table>
				</section>
