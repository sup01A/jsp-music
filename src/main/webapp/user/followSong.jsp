				...
				<section class="table-responsive">
					<table class="table table-striped">
						...
						<c:forEach items="${list }" var="Song" varStatus="status">
							<tr>
								<td>${status.count }</td>
								<td><a href="<%=contextPath %>/SongServlet?info=play&playId=${Song.songId}" target="_blank"><c:out value="${Song.songTitle }"></c:out> </a></td>
								<td><a href="<%=contextPath %>/SingerServlet?info=play&playId=${Song.singerId}" target="_blank"><c:out value="${Song.singer.singerName }"></c:out></a></td>
								<td><a href="<%=contextPath %>/SongServlet?info=removeFollow&songId=${Song.songId}">删除</a></td>
							</tr>
						</c:forEach>
					</table>
				</section>
